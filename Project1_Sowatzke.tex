...
\end{equation}
\par
The received signal is the sum of each target return and noise sampled from a complex normal distribution with unit power. During waveform transmission, the receiver is assumed to be "blanked" (i.e. it cannot listen while transmitting). During this period, the received signal is replaced with zeros.
\par
...
The matched filter uses coherent integration to increase the SNR of the received signal. The matched filter output SNR should be greater than the SNR of the received signal by a factor of $L$, where $L$ is the matched filter length in samples. For the given radar system, the matched filter length is
\begin{equation}
...
\begin{figure}[H]
\centerline{\fbox{\includegraphics[width=0.5\textwidth]{adc_sig.png}}}
...
\begin{figure}[H]
\centerline{\fbox{\includegraphics[width=0.5\textwidth]{mf_output.png}}}
