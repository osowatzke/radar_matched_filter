...
The matched filter uses coherent integration to increase the SNR of the received signal. The matched filter output SNR should be greater than the SNR of the received signal by a factor of $N$, where $N$ is the matched filter length in samples. For the given radar system, the matched filter length is
\begin{equation}
N = \frac{\tau}{T_s} = \tau f_s = 400 \text{ samples}
\end{equation}
...
\end{equation}
For this system, the SNR at the ADC is chosen to be 20 dB. This SNR corresponds to an RCS of
$0.01307m^2$.
