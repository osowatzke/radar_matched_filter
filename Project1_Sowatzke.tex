...
%The radar system's range resolution is related to the width of the peak of matched filter output. For a rectangular pulse, the matched filter response is triangular. In contrast, the response of a chirp function is a sinc. 
The range resolution of the matched filter output is theoretically given by
\begin{equation}
\Delta R = \frac{c}{2\beta}
\end{equation}
This result will be confirmed via simulation. First, a target will be placed as described in Section \ref{mf_snr_section}. Then, a second target will be placed at a larger range and moved toward the first target until the second return cannot be distinguished from the first.
